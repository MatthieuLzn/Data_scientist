...
count = 0
#puts journalists[1..7] #Appelle de la position 1 .. 7 incluses
#puts journalists.first #Appelle le premier 
#puts journalists.last #Appelle le dernier
#puts journalists.count #Compte le nombre d'élément dans l'array
#puts journalists.length
#puts journalists.max
#puts journalists.min
#puts journalists.sort #tri par ordre alphabétique 
...
def min_size(journalists)
journalists.min_by { |x| x.size }
puts "Le plus petit élément du tableau est : " + journalists.min_by { |x| x.size }
end



#Les emails avec 5 caractere 
    ...
    puts "Il y a #{count} email de 5 caractere"
    

count = 0
#puts journalists.sort_by(&:upcase) 
    journalists.each do |i|
        if i != "@epenser"
            count += 1
        end
        if i == "@epenser"
            puts "@epenser se trouve à la place n#{count}" 
            return count
        end
    end
